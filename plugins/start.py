...
async def add_user_premium_command(client: Bot, message: Message):
    while True:
        ...
        try:
            await Bot.get_users(user_ids=int(user_response.text))
            break
        except Exception:
    ...
    while True:
        try:
            time_response = await client.ask(
        ...
        if time_response.text.isdigit() and int(time_response.text) in [1, 2, 3, 4, 5]:
            timeforprem = int(time_response.text)
            break
        else:
            await message.reply("You have given the wrong input. 😖")
            continue
